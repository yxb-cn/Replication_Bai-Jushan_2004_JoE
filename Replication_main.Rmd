...
```{R eval=FALSE, include=FALSE}
axz_scale <- list(
        title = "10*log(employment)",range = c(-4, 4)
)
...
&emsp;&emsp;However, these criteria' accuracy depend on the choice of $kmax$ to some extent. If the $kmax$ is chosen to be too large, for example, $kmax=20$ of $kmax=30$ in section 5.1.2, then we will see the wrong results as follows.
\newpage

```{r echo=FALSE}
estimated_num2_kmax20 = matrix(0,13,6)
r2 = 2
p = 1
rou2 = 0.5
theta2 = 0.5
...
for(i in 1:repnum){
  for(j in 1:13){
    ...
    X = X_list$X
    X_diff = X_list$X_diff
    estimated_num2_kmax20[j,1:3] = estimated_num2_kmax20[j,1:3] + estimate_factornum_byPC_IPC(X_diff,kmax_large1,3,1)
    estimated_num2_kmax20[j,4:6] = estimated_num2_kmax20[j,4:6] + estimate_factornum_byPC_IPC(X,kmax_large1,3,2)
  }
}
estimated_num2_kmax20 = estimated_num2_kmax20/repnum
result2_kmax20 = cbind(NT_cases,estimated_num2_kmax20)
colnames(result2_kmax20) <- c("N","T","PC1","PC2","PC3","IPC1","IPC2","IPC3")
kable(result2_kmax20,booktabs = T,longtable = T,caption ="Estimated number of factors averaged over 1000 repititions (Generalized dynamic factor model) when kmax=20") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive","HOLD_position"),full_width = T)%>%
  add_header_above(c(" " = 2, "Differenced data" = 3, "Level data" = 3))%>%
  footnote(general = "The level-data method gives an estimate of r (true value r = 2), and the differenced-data method gives an estimate of r(p + 1) (true value is 4).", threeparttable = TRUE)
```

```{r echo=FALSE}
estimated_num2_kmax30 = matrix(0,13,6)
r2 = 2
p = 1
rou2 = 0.5
theta2 = 0.5
...
for(i in 1:repnum){
  for(j in 1:13){
    ...
    X = X_list$X
    X_diff = X_list$X_diff
    estimated_num2_kmax30[j,1:3] = estimated_num2_kmax30[j,1:3] + estimate_factornum_byPC_IPC(X_diff,kmax_large2,3,1)
    estimated_num2_kmax30[j,4:6] = estimated_num2_kmax30[j,4:6] + estimate_factornum_byPC_IPC(X,kmax_large2,3,2)
  }
}
estimated_num2_kmax30 = estimated_num2_kmax30/repnum
result2_kmax30 = cbind(NT_cases,estimated_num2_kmax30)
colnames(result2_kmax30) <- c("N","T","PC1","PC2","PC3","IPC1","IPC2","IPC3")
kable(result2_kmax30,booktabs = T,longtable = T,caption ="Estimated number of factors
      averaged over 1000 repititions (Generalized dynamic factor model) when kmax=30") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive","HOLD_position"),full_width = T)%>%
  add_header_above(c(" " = 2, "Differenced data" = 3, "Level data" = 3))%>%
  footnote(general = "The level-data method gives an estimate of r (true value r = 2), and the differenced-data method gives an estimate of r(p + 1) (true value is 4).", threeparttable = TRUE)
```

&emsp;&emsp;According to my analysis to the simulation results, in practical application, some error may occur in the following three conditions:

  1. $T$ and $N$ is too small. According to Table 3 and Table 4, small $T$ and $N$ tend to estimate the number of factors incorrectly.
  2. The high dimensional dataset contains I(d)(d$\geq2$) series. Since this paper only consider I(0) and I(1) process, if $X$ contains I(d)(d$\geq2$) series, the criteria tend to over estimate the number of factors. For example, I test the dataset in @mccracken2016fred without transforming to ensure stationarity, the estimated number equals to $kmax$ by these criteria in this paper, no matter what $kmax$ is set (in this example, $T=730$ and $N=136$, obviously this error is not caused by data size).
...
1     & Farms                                        & 31    & Trucking and warehousing1              \\
2     & Agricultural services, forestry, and fishing & 32    & Water transportation                   \\
3     & Metal mining                                 & 33    & Transportation by air1                 \\
